...
static GLFWwindow *CreateGLFWWindowWithFallback(int width, int height, const char *title)
{
	struct HintSet { int major; int minor; int profile; };
	const HintSet hints[] = {
		{ 3, 2, GLFW_OPENGL_CORE_PROFILE },
		{ 3, 3, GLFW_OPENGL_CORE_PROFILE },
		{ 4, 1, GLFW_OPENGL_CORE_PROFILE },
	...
	for (const auto &h : hints)
	{
		...
		if (h.major > 0)
		{
			glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, h.major);
			glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, h.minor);
			glfwWindowHint(GLFW_OPENGL_PROFILE, h.profile);
		}
		...
		if (win)
		{
			if (h.major > 0)
			{
				fprintf(stderr, "SpaceCal: Created OpenGL %d.%d %s context\n",
					h.major, h.minor,
					h.profile == GLFW_OPENGL_CORE_PROFILE ? "core" :
					h.profile == GLFW_OPENGL_COMPAT_PROFILE ? "compat" : "any");
			}
			else
			{
				fprintf(stderr, "SpaceCal: Created OpenGL any-available context\n");
			}
			return win;
		}
	}
...
void CreateGLFWWindow()
{
	...
	glfwWindow = CreateGLFWWindowWithFallback(fboTextureWidth, fboTextureHeight, "Space Calibrator");
	if (!glfwWindow)
	...
	// Query actual GL version and build matching GLSL string for ImGui
