...
static void *DetourGetGenericInterface(vr::IVRDriverContext *_this, const char *pchInterfaceVersion, vr::EVRInitError *peError)
{
	...
	else if (iface.rfind("IVRServerDriverHost_", 0) == 0)
	{
		LOG("WARNING: Unknown IVRServerDriverHost version '%s'. Space Calibrator may need an update to support this SteamVR version.", pchInterfaceVersion);
	}
